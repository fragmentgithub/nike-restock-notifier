...
2026-09-05に本番監視をCloudflareへ移行しました。本人限定ページと長期保存も本番へ反映し、PCを停止しても監視・保存・閲覧を継続します。旧方式で保存した入荷19件は削除せず保全し、検証済みトレンドからは除外しました。本人の実ログイン後の画面確認は未実施です。旧GitHub Pagesの公開は停止し、GitHubにはコード・CI・認証付きの外側health確認を残しています。詳細は [HANDOFF.md](HANDOFF.md) を参照してください。
...
長期保存の開始時は、残っている全体履歴と商品別履歴を取り込みます。過去の全履歴が揃うわけではありません。保存開始日とこの制約をページに表示します。判定方式を更新した場合は、検証済み記録の開始時刻も表示し、それ以前の行を保存したまま集計対象から外します。実際の補充時刻や将来の入荷確率を示すものではありません。

分析は、現行の在庫判定で検証できた観測だけを使います。以前から保存されていた入荷イベントに監視時間を推測して足すことはありません。曜日と時間帯の表は入荷検出件数と100商品時間あたりの頻度を併記し、監視時間がない場所を「未観測」、監視済みで入荷がなかった場所を0件として区別します。
...
- Nikeの商品ページを取得できない場合の代替APIを現行のフィルター・GTIN在庫形式へ対応させ、日本サイズで通知対象を照合します。不明・矛盾した在庫は通知せず、商品全体の販売終了・発売前を優先します。
- 全有効商品の継続的な取得失敗、Discordの連続送信失敗、監視完了時刻の遅延を異常判定と本人限定ページへ反映します。
- 時間帯や集計期間の境界に残る短い観測時間を0へ丸めず、グラフ表示が失敗する問題を修正しました。期間比較の最低監視時間は丸め前の値で判断します。
...
- Nikeの商品データにある `ACTIVE` を在庫ありとは扱わず、候補がある場合は公式のサイズ別在庫APIで確認します。APIやSKU対応が不明なら通知せず、明示できるまで待ちます。
- 在庫APIが一時停止した場合に商品ページの購入ボタンを使うのは、表示中カラーと監視対象カラーが一致するときだけです。
- 旧方式の入荷19件は削除せず保全し、検証開始時刻より前の記録をグラフ・頻度・売り切れ時間・期間比較から除外しました。
- 失敗したバックアップの未完成世代を片付け、最新バックアップの時刻と連続失敗をhealthへ含めました。非公開ページは監視エラーを次の再読込で表示します。
...
CIでも同じ確認を行います。`cloudflare:test` は隔離したCloudflare実行環境で、Access認証、読み取り専用の接続、長期SQL集計、SQLiteへの1万件の観測保存、別DOへのバックアップと復元、通知済み情報の保持を検証します。本番キーを使わず、NikeやDiscordへの通信は行いません。2026-09-19の変更は334テストと実Workerdでの認証・接続検証を通過し、本番へ反映済みです。
