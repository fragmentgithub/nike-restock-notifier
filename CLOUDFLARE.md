...
商品ページから取得できない場合は、商品カラーを指定したNikeの商品feedへ切り替えます。現行の `availableGtins` はSKUのGTINで照合し、配送方式・市場・カラーの違う在庫を混ぜません。従来の `availableSkus` も利用できます。日本向けの `countrySpecifications.localizedSize` をサイズ表示と対象サイズの照合へ使い、在庫が欠落・不明・矛盾する場合は通知しません。

`/healthz` は処理の起動だけでなく、商品取得・通知配送・完了時刻も検査します。休止や無効化していない全商品で各2回以上確認に失敗した場合、またはDiscord送信に2回連続で失敗した場合を異常とします。商品取得と通知配送の失敗回数は保存され、再起動後も引き継ぎます。通知の異常は実際の送信成功で解除します。完了時刻には通常15分以上の猶予を設け、設定された確認間隔や全商品休止時の探索・再確認間隔に合わせます。本人限定ページにも一般化したエラーを表示し、外側healthへ商品や履歴は返しません。

長期集計の観測時間は、時間帯や期間の境界で1秒未満になっても正の値を保持します。未観測は `null`、観測済みで入荷がない場合は0件です。30日比較の最低24商品時間は表示用に丸める前の実時間で判定します。
...
2026-09-19の変更は334テストと実Workerdの認証・service binding・SQL集計・別DOバックアップ復元検証を通過しています。実Workerd検証では、本番の秘密値を使わず外部通信を遮断します。
