...
- 監視方式: **ループ方式**。全商品を順番に確認し、巡回完了後に `INTERVAL_SECONDS` 秒待つ。取得失敗が続く場合は最大10分まで自動バックオフする。
...
- `.github/workflows/pages.yml`
  ...
  - **自己連鎖**: 待機runがない場合だけ `gh workflow run` で次を起動する。待機runがあれば追加せず、concurrencyによる不要なキャンセルを避ける。
...
- `INTERVAL_SECONDS`: 任意。全商品巡回後の待機秒。30〜1800、デフォルト120。**現在120を設定。** 取得失敗時は最大10分まで自動バックオフする
...
- **Nikeのレート制限リスク:** 商品ごとのアクセス間隔と巡回後待機に加え、失敗時は最大10分まで自動バックオフする。
