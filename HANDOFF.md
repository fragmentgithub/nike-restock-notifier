...
- `.github/workflows/pages.yml`
  ...
  - **自己連鎖**: 各実行の最後に `gh workflow run` で自分自身を再ディスパッチ(`if: always()`)。cronの発火遅延(実測65〜80分の日もある)に依存せず連続監視できる。実行が異常終了(runner確保失敗など)して連鎖が切れた場合はcronがバックアップとして再開する
...
- **スケジュール遅延:** cronの発火は5分設定でも実測20〜80分間隔(日によって大きく変動)。GitHubの仕様であり異常ではない。自己連鎖があるため通常運用ではcronに依存しないが、連鎖が切れた後の再開はcron次第なので最大1時間強の空白が出うる。
