...
- 監視方式: **ループ方式**。1回のActions実行が約25分間動き続け、その中で約2分ごとにNikeをチェックする(下記Architecture参照)。cron自体は5分設定でGitHub側で20〜40分に遅延するが、ループ中は細かくチェックされるため、通知の実効間隔は約2分
...
- `scripts/monitor.js`(**ループ方式**)
  ...
  - 読み込み:
    ...
    - `INTERVAL_SECONDS` variable(任意、チェック間隔秒。60〜1800、デフォルト120)
...
- `INTERVAL_SECONDS`: 任意。ループ内の**実際のチェック間隔(秒)**。60〜1800、デフォルト120
